          ...
          <section class="overflow-hidden rounded-xl border border-surface-100 bg-white shadow-sm">
            ...
            <div class="grid grid-cols-1 gap-4 p-5 lg:grid-cols-2">
              ...
              <label class="block text-sm font-medium text-surface-800 lg:col-span-2">Philippine discount eligibility
                <select name="discount_scheme" {{ $isStaffOrAdmin ? '' : 'disabled' }} class="mt-2 h-10 w-full rounded-lg border border-surface-200 px-3 text-base font-normal outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
                  <option value="none" @selected(old('discount_scheme', $service->discount_scheme) === 'none')>None — ordinary landscaping service</option>
                  <option value="statutory_20_vat_exempt" @selected(old('discount_scheme', $service->discount_scheme) === 'statutory_20_vat_exempt')>20% + VAT exemption — confirmed qualifying service</option>
                </select>
