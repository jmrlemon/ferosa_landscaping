...
<head>
  ...
  <title>Add Service - Ferosa Landscaping</title>
...
<body class="min-h-screen bg-surface-100 text-surface-900 font-sans antialiased">
  ...
  <header class="h-14 bg-white border-b border-surface-200 flex items-center justify-between px-5">
    <h1 class="text-sm font-semibold text-surface-600">Add Service</h1>
  ...
  <main id="admin-main" tabindex="-1" class="p-5">
    ...
    <div class="mb-5 flex items-center gap-4">
      ...
      <div>
        <h2 class="text-2xl font-bold text-brand-950">Add New Service</h2>
        <p class="mt-1 text-sm text-surface-500">Create a customer-bookable service for scheduling and cost estimation.</p>
      </div>
    </div>

    <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
      <div class="xl:col-span-2">
        <form id="add-service-form" method="POST" action="{{ route('admin.services.store') }}" class="space-y-6">
          @csrf
          <section class="overflow-hidden rounded-xl border border-surface-100 bg-white shadow-sm">
            <div class="border-b border-surface-200 px-5 py-4 flex items-center justify-between gap-3">
              ...
              <span class="rounded-full bg-brand-50 px-2.5 py-1 text-xs font-semibold text-brand-700">Bookable service</span>
            </div>
      ...
      <aside class="h-fit rounded-xl border border-surface-100 bg-white p-4 shadow-sm">
        ...
        <div class="mt-4 rounded-lg border border-brand-100 bg-brand-50 px-3 py-3">
          ...
          <p class="mt-1 text-xs leading-5 text-brand-700">Use a clear service name and realistic default fee so booking and estimation stay consistent.</p>
        </div>
        <button type="submit" form="add-service-form" class="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-brand-700 py-2.5 text-base font-semibold text-white transition-colors hover:bg-brand-800">Add Service</button>
