          ...
          <section class="overflow-hidden rounded-xl border border-surface-100 bg-white shadow-sm">
            ...
            <div class="grid grid-cols-1 gap-4 p-5 lg:grid-cols-3">
              ...
              <label class="block text-sm font-medium text-surface-800 lg:col-span-3">Philippine discount eligibility
                <select name="discount_scheme" {{ $isStaffOrAdmin ? '' : 'disabled' }} class="mt-2 w-full rounded-lg border border-surface-200 px-3 py-2 text-base font-normal outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
                  <option value="none" @selected(old('discount_scheme', $product->discount_scheme) === 'none')>None — normal Ferosa item</option>
                  <option value="bnpc_5" @selected(old('discount_scheme', $product->discount_scheme) === 'bnpc_5')>5% BNPC — confirmed listed basic/prime commodity</option>
                  <option value="statutory_20_vat_exempt" @selected(old('discount_scheme', $product->discount_scheme) === 'statutory_20_vat_exempt')>20% + VAT exemption — confirmed statutory item</option>
                </select>
